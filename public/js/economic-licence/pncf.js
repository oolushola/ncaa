$(function($){
    ...
    $("#addPncf").click(function(event){
        event.preventDefault();
        if(validatPncfRequest()==false) return;
        $("#frmPncf").submit();
    });

    $("#updatePncf").click(function(event){  
        event.preventDefault();
        if(validatPncfRequest()==false) return;
        $("#frmPncf").submit();
    });
    ...
    $("#frmPncf").ajaxForm(function(data) {
        if(data == 'exists') {
            ...
            if(data == 'saved' || data == "updated") {
                ...
                window.location = '/economic-licence/pncf';
            }
        }
    })
    ...
    // change function for pncf operators sort
    $("#operator").change(function() {
        $operator = $("#operator").val();
        $("#contentDropper").html('<img src=\'/images/ajax.gif\'>please wait...');
        $.get('/pncf-operator-sort', {direction:$operator}, function(data) {
    ...
    $("#chooseStatus").on("change", function(){
        ...
        if($status == 'active'){
            $.get('/pncf-active', {active:$status}, function(data){
                $("#contentDropper").html(data).removeClass('error');
            });    
        }

        if($status == 'expired'){
            $.get('/pncf-expired', {expired:$status}, function(data){
                $("#contentDropper").html(data).removeClass('error');
            });    
        }

        if($status == 'expiringSoon'){
            $.get('/pncf-expiring-soon', {expiringSoon:$status}, function(data){
                $("#contentDropper").html(data).removeClass('error');
            });    
        }
    });

    $("#downloadPNCF").click(function(){
        var name = Math.random().toString().substring(7);
        $("#exportTableData").table2excel({
            filename:`none-commercial-flights-${name}.xls`
        });
    });
