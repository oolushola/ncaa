...
@section('title')Non Commercial Flight @stop

@section('main')
    <div class="page-header">
        <h3 class="page-title">
            None Commercial Flight 
        </h3>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="{{URL('economice-licence/pncf/view')}}">View All</a></li>
            <li class="breadcrumb-item active" aria-current="page">@if(isset($recid))Update @else Add @endif PNCF</li>
            </ol>
        </nav>
    </div>
    <div class="row">
        <div class="col-md-5 grid-margin ">
            ...
            <div class="card-body">
                @if(isset($recid)) 
                <form name="frmPncf" id="frmPncf" method="POST" action="{{URL('economic-licence/pncf',$recid->id)}}">
                {{ method_field('PATCH') }}
                @else
                <form name="frmPncf" id="frmPncf" method="POST" action="{{URL('/economic-licence/pncf')}}">
                @endif
                ...
                {!! csrf_field() !!}
                    ...
                    <div class="form-group">
                        ...
                        <input type="file" name="pncf_certificate" id="file" style="font-size:12px; display:inline-block"  @if(isset($recid))disabled  @endif > @if(isset($recid)) 
                        <span class="labelholder" id="changePhoto" style="cursor:pointer">Change</span> @endif
                        <input type="hidden" name="photoChecker" id="photoChecker" value="@if(isset($recid)){{$recid->pncf_certificate}}@else{{0}}@endif">
                    </div>
                    ...
                    @if(isset($recid))        
                    <button type="submit" class="btn btn-gradient-primary mr-2" id="updatePncf">UPDATE</button>
                    @else
                    <button type="submit" class="btn btn-gradient-primary mr-2" id="addPncf">SAVE</button>
                    @endif
                        ...
                        <table class="table table-bordered" width="100%">
                            <thead class="table-info">
                                <tr class="table-warning">
                                    <th colspan="8" style="font-size:11px; font-weight:bold">PNCF Lists</th>
                                    {!! $pncflistings->links() !!}
                                </tr>
                                <tr>
                            ...
                            <tbody>
                                ...
                                <?php $count = 0; ?>
                                    ...
                                    <?php $count++; 
                                        ...
                                        $convertdate_of_first_issue = strtotime($pncf->date_of_first_issue);
                                        $convertdaterenewal = strtotime($pncf->date_of_renewal);
                                    ...
                                    ?>
                                        <tr class="{{$css}}">
                                            <td>{{$count}}</td>
                                            <td>{{strtoupper($pncf->licence_no)}}</td>
                                            <td>{{$date_of_first_issue}}</td>
                                            <td>{{$renewal}}</td>

                                            @if(Auth::user()->role == 1 || Auth::user()->role == 2)
                                            <td style="color:blue" class="center">
                                                <a href="{{URL('economic-licence/pncf/'.$pncf->id.'/edit')}}">
                                                    <i class="mdi mdi-pen" style="font-size:25px;"></i>
                                                </a>
                                            </td>
                                            
                                            <td style="color:red" class="center">
                                                <form method="POST" name="frmDeletePncf" id="frmDeletePncf">
                                                    {!! csrf_field() !!} {!! method_field('DELETE') !!}
                                                    <i class="mdi mdi-delete-forever deletePncf" style="font-size:25px; cursor:pointer" value="{{$pncf->id}}" title="Delete {{$pncf->licence_no}}"></i>
                                                </form>
                                            </td>
                                            @endif
                                            @if(Auth::user()->role == 1)
                                            <td>{{$pncf->created_by}}</td>
                                            @endif
